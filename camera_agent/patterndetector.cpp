		...
		if (per>(avsize/4) && per<(4*avsize)) {
			...
			if (polycont.size()==4 && isContourConvex(Mat (polycont))){
				...
				if (retvalue>0) {
					...
					switch (patCand.orientation) {
                        case 0:
                            patCand.orientationStr = 'N';
                            break;
                        case 1:
                            patCand.orientationStr = 'E';
                            break;
                        case 2:
                            patCand.orientationStr = 'S';
                            break;
                        case 3:
                            patCand.orientationStr = 'O';
                            break;
                    }
					...
					float cx = (patCand.vertices.at(0).x + patCand.vertices.at(2).x)/2;
                    float cy = (patCand.vertices.at(0).y + patCand.vertices.at(2).y)/2;
                    
                    patCand.center = cvPoint(cx, cy);
